...
#+begin_src emacs-lisp :tangle no
  (require 'use-package)
  (setq use-package-always-ensure t) ;; Install a package if it isn't installed already
...
**** evil-commentary
[[https://github.com/linktohack/evil-commentary][evil-commentary]] is a port from tpope's vim-commentary.
...
=yarn global add= adds symlinks to the binaries it installs to =.local/bin=. This is good because it mantains the installation at a user level. =lsp-mode= finds lsp server executables by executing =executable-find=, which searches in the folders listed in =exec-path=.
