...
See [[info:emacs#Init Rebinding][emacs#Init Rebinding]]
...
*NOTE:* Currently trying out just using =Vertico=, along with the new (built-in) =completion-preview-mode=!
...
This snippet adds a hook to =org-mode= buffers so that any Org file in the =.dotfiles= folder gets auto tangled on save.
...
I use the built-in package =flymake= for linting.
...
For debugging, I'm currently using the built-in GUD interface.
...
I'm still using [[https://emacs-tree-sitter.github.io/][tree-sitter]], even though the built-in integration has already shipped to the latest official releases. I'll probably change to =treesit= whenever I have more free time available.
...
This snippet ensures =treesit= looks for grammars installed through the default Guix profile:
#+begin_src emacs-lisp
...
Some small enhancements to compilation (=M-x compile=) buffers.
...
Obs: currently using built-in solution: ~dired-do-open~
...
[[https://github.com/luishgh/competitive-companion.el][competitive-companion.el]] is a package of my own 🤩. It provides an Emacs integration to the famous [[https://github.com/jmerle/competitive-companion][Competitive Companion browser extension]].
...
[[https://github.com/luishgh/rbx.el][rbx.el]] is another package of my own, which provides Emacs integration for [[https://rbx.rsalesc.dev/]].
